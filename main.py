...
def dfs_route(route_config, path):
    for key, value in route_config.items():
        if isinstance(value, dict):
            ...
            dir = "/" + "/".join(path + [key])
            try:
              os.makedirs('content/post/' + dir + '/')
            except Exception as e:
                print('已存在目录', 'content/post/' + dir + '/')
...
<div>
版权声明：
本文为 {tag} 原创文章，本站仅在原基础上进行总结，受 CC 4.0 BY-SA 版权协议保护。未经授权，禁止转载。
...
try:
    with open('content/post/' + dir + '/' + filename + '.md', mode='w', encoding='utf-8') as f:
        if "'" in title:
            title = '"' + title + '"'
        else:
            title = "'" + title + "'"
    ...
    error_line = e.__traceback__.tb_lineno
    error_info = '第{}行发生error为: {}'.format(error_line, str(e))
    print(error_info)
...
print("--------- 解析文章 begin ---------")
解析文章()
print("--------- 解析文章 end ---------")# From https://github.com/MHG-LAB/RSSBOX/
